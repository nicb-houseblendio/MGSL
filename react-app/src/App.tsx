        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          {isARCH && archSource === 'netsuite' && (
            <>
              ...
              {/*
                HOW OLD the figures are, which the ARCH grid had no way of saying.
                The shared badge below is gated `!isARCH` because its bound calls
                anything over an hour stale and this cache rebuilds hourly, so it
                would have read amber every cycle. ARCH gets its own bounds instead
                of nothing: see lib/archFreshness. A trader who cannot tell a stale
                screen from a wrong one has to assume their stock vanished, which is
                what Marc-Antoine reported on 2026-09-08.
              */}
              {(() => {
                const f = archFreshness(archMeta?.lastUpdated, Date.now());
                const bg =
                  f.state === 'fresh'
                    ? 'rgba(76,175,80,0.25)'
                    : f.state === 'due'
